...
import { Plus, X, Loader2, Check, AlertCircle, Code, Gift } from "lucide-react";
...
interface CreatePoolModalWithReferrerProps {
  onClose: () => void;
  onSuccess: () => void;
...
export default function CreatePoolModalWithReferrer({ 
  onClose, 
  onSuccess, 
  referrerWallet = null,
...
useEffect(() => {
  ...
  const handleCreatePool = async () => {
    ...
    try {
      ...
      if (poolExists) {
        throw new Error("Maximum number of pools (10) reached for this token.");
      }
      ...
      const referrerPubkey = referrerWallet ? new PublicKey(referrerWallet) : null;
      ...
      const initParams = {
        ...
        referrerSplitBps: referrerPubkey ? new anchor.BN(referrerSplitBps) : null,
        enableReflections: poolConfig.enableReflections,
        reflectionToken: reflectionTokenMintToUse,
      };

      console.log("🔗 Referrer config:", {
        referrer: referrerPubkey?.toString() || "none",
        splitBps: referrerSplitBps,
      });
      ...
      if (poolConfig.enableReflections && reflectionTokenMintToUse) {
        ...
        if (isNativeSol) {
          ...
          initPoolAccounts.reflectionTokenAccount = projectPDA;
      ...
      if (poolConfig.enableReflections) {
        try {
          ...
          console.error("Could not calculate reflection vault:", err);
      ...
      console.log("✅ Pool saved with referrer:", savedPool);
  ...
  return (
    ...
    <div className={`fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto ${showSuccessModal ? 'hidden' : ''}`}>
      <div className="bg-black/90 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-white/[0.05]">
        ...
        {referrerWallet && (
          <div className="mx-6 mt-4 p-4 bg-[#fb57ff]/10 border border-[#fb57ff]/30 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold" style={{ color: '#fb57ff' }}>You were referred!</p>
                <p className="text-xs text-gray-300 font-mono">{referrerWallet.slice(0, 8)}...{referrerWallet.slice(-6)}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-400">Referrer earns</p>
                <p className="text-sm font-semibold" style={{ color: '#fb57ff' }}>{referrerSplitBps / 100}% of fees</p>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 bg-white/[0.02] border border-white/[0.1] rounded-lg text-white focus:outline-none transition-colors focus:border-[#fb57ff]/50"
                  placeholder="1000"
                  min="0"
                />
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 p-3 bg-white/[0.02] border border-white/[0.1] rounded-lg text-white focus:outline-none transition-colors focus:border-[#fb57ff]/50"
                    placeholder="0"
                    min="0"
                    max="100"
                  />
                  ...
                  <div className="space-y-4 pt-2 border-t border-white/[0.05]">
                    ...
                    {poolConfig.reflectionType === "external" && (
                      <div>
                        ...
                        <input
                          ...
                          className="w-full p-3 bg-white/[0.02] border border-white/[0.1] rounded-lg text-white focus:outline-none transition-colors focus:border-[#fb57ff]/50 font-mono text-sm"
                    ...
                    <div className="bg-white/[0.02] border border-white/[0.1] rounded-lg p-3">
                      <p className="text-xs text-gray-400 leading-relaxed">
                        ...
                        <strong>• Native SOL:</strong> Use mint address So11111111111111111111111111111111111111112
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="flex gap-4">
                <button
                  ...
                  disabled={loading}
                  className="flex-1 px-6 py-3 bg-white/[0.05] hover:bg-white/[0.08] border border-white/[0.05] hover:border-[#fb57ff]/30 rounded-lg font-semibold transition-colors disabled:opacity-50"
                >
                  Back
                </button>
                <button
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/[0.02] border border-white/[0.05] rounded-lg p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {/* Show referrer info in confirmation */}
                  {referrerWallet && (
                    <>
                      <div className="col-span-2 border-t border-white/[0.05] pt-4 mt-2"></div>
                      <div className="text-gray-400 text-sm flex items-center gap-2">
                        <Gift className="w-4 h-4" style={{ color: '#fb57ff' }} />
                        Referrer:
                      </div>
                      <div className="text-white font-mono text-xs text-right">{referrerWallet.slice(0, 6)}...{referrerWallet.slice(-4)}</div>
                      
                      <div className="text-gray-400 text-sm">Referrer Split:</div>
                      <div className="font-medium text-sm text-right" style={{ color: '#fb57ff' }}>{referrerSplitBps / 100}% of fees</div>
                    </>
                  )}
